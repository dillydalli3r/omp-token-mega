...
| --- | --- |
| [docs.lithosai.com](https://docs.lithosai.com) (`/index`, `/authentication`, `/rate-limits`, `/billing`, `/coding-agents/omp`) | The base URL (`https://api.lithosai.cloud/v1`), the bearer-key scheme and console key page, the `LITHOSAI_API_KEY` convention, the model id and limits used by the static catalogue, the `x-ratelimit-*` / `retry-after-ms` / `x-should-retry` header set, the "balance, not a countdown" reading of the per-minute buckets, and the three per-million rates (input, cached input, output) behind the `lithos.*PerMillion` settings. |
| [docs.lithosai.com/openapi.yaml](https://docs.lithosai.com/openapi.yaml) | The endpoint list (`/models`, `/models/{author}/{slug}`, `/chat/completions`), which is why the plugin discovers models from `/models` and reports budgets from response headers: there is no billing or usage endpoint to poll. |
