...
```
DS cache 92% · 120k cached · $0.005892 saved · DS ¥11.48 · used $0.0042 · TS -28.8 KB (~7.4k tok) · 1/1 results
...
are now one plugin with one key, one line, and segments that are dropped whole when they do not
fit rather than clipped mid-number.
...
| **Cache accounting** | Measures the DeepSeek prefix cache: hit rate, cached tokens, USD saved, miss attribution, tool-catalogue drift, subagent shards. Read-only on the request path. | DeepSeek |
...
| `/mega status` | The status row, as text, for modes that hide the footer. |
...
`/models`, and stores it. `LITHOSAI_API_KEY` in the environment works too and needs no login.
Create keys at <https://console.lithosai.cloud/keys>.

**Models.** The declared catalogue is discovered from `GET /models` with the stored key, with
`moonshotai/Kimi-K3` — the model LithosAI's own omp guide names — as the offline fallback.
Their `/models` response carries ids and owners only, so discovered models are registered with
the limits from that guide (256K context, 32K output); put different ones in `models.yml` if
your console shows something else.
